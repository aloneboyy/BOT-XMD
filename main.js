...
function arslanLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [KAIRO-XMD] ${new Date().toISOString()}: ${message}`);
}
    ...
    try {
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const newsletterJids = ['120363413253579833@newsletter'];
                ...
                const myquoted = {
                    key: { remoteJid: 'status@broadcast', participant: '50955034382@s.whatsapp.net', fromMe: false, id: createSerial(16).toUpperCase() },
                    message: { contactMessage: {
                        displayName: '© SIDD-MD',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:SIDD-MD BOY\nORG:SIDD-ALONNE BOY;\nTEL;type=CELL;type=VOICE;waid=50955034382:50955034382\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© SIDD-MD' } }
                    }},
        ...
        arslanLog(`KAIRO-XMDI Pair error: ${err.message}`, 'error');
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🤳 KAURO-XMD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
